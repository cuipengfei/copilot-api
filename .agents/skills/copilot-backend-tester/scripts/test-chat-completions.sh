...
usage() {
  cat <<'EOF'
Usage: test-chat-completions.sh LIVE_MODEL_ID [options]
...
body="$(
  jq -nc \
    ...
    '{model: $model, max_tokens: 1024, stream: $stream, messages: [{role: "user", content: $prompt}]}'
)"
...
headers=(
  ...
  -H "x-request-id: chat-$(cat /proc/sys/kernel/random/uuid)"
)
...
if [[ "$STREAM" == "true" ]]; then
  curl -sSN -D "$header_file" -o "$body_file" \
    "$COPILOT_BASE/chat/completions" "${headers[@]}" -d "$body"
else
